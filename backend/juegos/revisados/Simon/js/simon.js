...
console.log("[Simon] Arrancando...");
let socketAbierto = new Promise((resolve, reject) =>{
    // Escuchar eventos de la conexión WebSocket
    ws.onopen = function() {
        console.log('\n\n\n[Simon] Conectado al servidor WebSocket\n\n\n');
        //ws.send(JSON.stringify({ action: 'getClasificacion' }));
		ws.send(JSON.stringify({type: "juego", data: "Simon"}));
    };
}) 
		...
		function perder()
			{
				...
				ban=1; //Cambia el valor de bandera para cambiar el mensaje de menu.
